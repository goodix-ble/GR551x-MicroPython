...
{
    GAP_NO_SYNC      = 0x00,  /**< No attempt is made to synchronize to the periodic advertising. */
...
{
    GAPM_AOD_TX_1_US          =  (1 << 0),  /**< 1 us switching supported for AoD transmission. */
    GAPM_AOD_RX_1_US          =  (1 << 1),  /**< 1 us switching supported for AoD reception. */
    GAPM_AOA_RX_1_US          =  (1 << 2),  /**< 1 us switching and sampling supported for AoA reception. */
};
...
{
    SLOT_DURA_1US      = 0x01,  /**< Switching and sampling slots are 1us each. */
    SLOT_DURA_2US      = 0x02,  /**< Switching and sampling slots are 2us each. */
};
...
typedef struct
{
    ...
    uint8_t     cte_len;                   /**< CTE length (in 8us unit, range 0x02 to 0x14). */
...
typedef struct
{
    uint16_t skip;                       /**< Number of periodic advertising that can be skipped after a successful receive. 
                                              Maximum authorized value is 499. */
    uint16_t sync_to;                    /**< Synchronization timeout for the periodic advertising (in unit of 10ms between 100ms and 163.84s). */
    uint8_t  conidx;                     /**< Connection index used for periodic sync info reception. */
    uint8_t  mode;                       /**< Periodic advertising sync information reception mode, @see gap_per_adv_sync_info_rec_mode. */
    uint8_t  cte_type;                   /**< Type of Constant Tone Extension device should sync on. (@see gap_sync_cte_type). */
...
typedef struct
{
    ...
    uint16_t                conn_to;            /**< Timeout for automatic connection establishment (in unit of 10ms).
                                                     Cancel the procedure if not all indicated devices have been connected when the timeout occurs.
 ...
 ****************************************************************************************
 */
void ble_gap_per_adv_list_clear(void);
 ...
 ****************************************************************************************
 */
uint16_t ble_gap_per_sync_trans_param_set(uint8_t per_sync_idx, gap_per_sync_trans_param_t* p_per_sync_trans_param);

/**
 ****************************************************************************************
 * @brief Set the default parameters used for periodic sync transfer.
 * @param[in] p_def_per_sync_trans_param: Default periodic synchronization transfer parameters.
 *
 * @retval ::SDK_SUCCESS: Operation is successful.
 * @retval ::SDK_ERR_POINTER_NULL: Invalid pointer supplied.
 * @retval ::SDK_ERR_NO_RESOURCES: Not enough resources.
 ****************************************************************************************
 */
uint16_t ble_gap_def_per_sync_trans_param_set(gap_def_per_sync_trans_param_t* p_def_per_sync_trans_param);
